...
export interface Player {
  ...
  buildings: { building: Building; colonists: number }[];
  resources: Record<Resource, number>;
  shippedGoods: { resource: Resource; amount: number }[];
}
...
export interface RoleState {
  available: Role[];
  taken: { playerId: string; role: Role }[];
...
export interface GameState {
  ...
  ships: { capacity: number; cargo: Resource | null; amount: number }[];
...
export interface ServerToClientEvents {
  ...
  roleSelected: (data: { playerId: string; role: Role }) => void;
  actionPerformed: (data: { playerId: string; action: GameAction }) => void;
