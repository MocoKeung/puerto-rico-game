...
export default function SettlerPanel() {
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          <div className="animate-bounce mb-3"><Leaf size={40} className="text-emerald-500 mx-auto" strokeWidth={1.5} /></div>
          <p className="text-amber-600 font-medium">
            {t('settler.opponentChoosing', { name: player?.name })}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div>
      <div className="mb-4">
        <h2 className="text-lg font-bold text-amber-900">{t('settler.title')}</h2>
        <p className="text-sm text-amber-600">
          {t('settler.instructions')}
          {isRolePicker && t('settler.settlerBonus')}
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
        ...
        {canTakeQuarry && (
          ...
          >
            ...
            <span className="text-sm font-bold text-gray-700">{t('resources.quarry')}</span>
            <span className="text-[10px] text-gray-500">{t('settler.quarryDesc')}</span>
          </button>
        )}
      </div>
      ...
      {quarriesRemaining > 0 && (
        <p className="mt-3 text-xs text-amber-500">
          {t('settler.quarriesLeft', { n: quarriesRemaining })}
        </p>
      )}
    </div>
  );
